...
from GNNS.opf_functions import constraint_feasibility
...
class PDFeasNetworkV3(nn.Module):
    def __init__(self, constraint_dict, num_edges, num_feas_edges,
                 num_nodes, batch_size, optim_lst, feas_lst,  convergence_tol, lookup_helper, first_constr, proj_dict = {},  **kwargs):
        super(PDFeasNetworkV3, self).__init__()
        self.conv_tol = convergence_tol
        self.num_edges = num_edges
        self.num_nodes = num_nodes
        self.batch_size = batch_size
        self.feas_threshold = 0
        self.num_feas_edges = num_feas_edges
        self.lookup_helper = lookup_helper
        self.first_constr = first_constr
        self._build_network_(constraint_dict, proj_dict,  optim_lst, feas_lst)

    def _build_network_(self, constraint_dict, proj_dict,  optim_lst, feas_lst):
        self.optim_lst = nn.ModuleList()
        self.optim_norm = nn.ModuleList()
        self.feas_lst = nn.ModuleList()
        self.feas_norm = nn.ModuleList()
        for feas in feas_lst:
            feas_layer = PDConv2(constraint_dict, 
                    num_edges = self.num_feas_edges, itrs = feas[0], alpha = feas[1], convergence_tol=self.conv_tol, 
                            lookup_helper=self.lookup_helper, proj_dict=proj_dict)
            self.feas_lst.append(feas_layer)
            self.feas_norm.append(nn.LayerNorm(self.num_feas_edges))
        for optim in optim_lst:
            optim_layer = PDConv2(constraint_dict, 
                           num_edges = self.num_edges, itrs = optim[0], alpha = optim[1], 
                           convergence_tol=self.conv_tol, lookup_helper=self.lookup_helper, proj_dict=proj_dict)
            self.optim_lst.append(optim_layer)
            self.optim_norm.append(nn.LayerNorm(self.num_edges))
    ...
    def forward(self, x, edge_index, f_edge_index):
        for (layer, norm_layer) in zip(self.optim_lst, self.optim_norm):
            x = layer(x, edge_index)
            # layer.edge_weight = torch.sigmoid(layer.edge_weight)
            if constraint_feasibility(x, self.first_constr) > self.feas_threshold:
                for (feas_layer, feas_norm) in zip(self.feas_lst, self.feas_norm):
                    x = feas_layer(x, f_edge_index)
                    # feas_layer.edge_weight = torch.sigmoid(feas_layer.edge_weight)
                    if constraint_feasibility(x, self.first_constr) < self.feas_threshold:
...
class PDConv2(MessagePassing):
    def __init__(self, constraint_dict, num_edges, itrs, 
                 alpha, convergence_tol, lookup_helper, proj_dict = {}, scale = 1, **kwargs,):
        kwargs.setdefault('aggr', 'add')
        super().__init__(**kwargs)
        self.constr_dict = constraint_dict
        self.proj_dict = proj_dict
    ...
    def forward(self, x, edge_index):
        ...
        for _ in range(self.num_itrs):
            # norm_weight = torch.tanh(self.edge_weight)
            norm_weight = self.edge_weight
            x = self.propagate(edge_index, x=x, edge_weight = norm_weight, edge_scale = self.edge_scale, vals = x)
            self.prev_vals.append(x[:, 1].detach().cpu().numpy().tolist())
        # return feas_point if feas_point is not None else x
        return x

    def message(self, x_i, x_j, vals, edge_weight, edge_scale):
        msg = torch.zeros_like(edge_weight)
        for (ind,(i, j, e, s)) in enumerate(zip(x_i, x_j, edge_weight, edge_scale)):
            i_label = int(i[0].item())
            j_label = int(j[0].item())
            msg[ind] = s * e * self.constr_dict[j_label][i_label](j.tolist(), i.tolist(), vals, self.lookup_helper)
            if torch.isnan(msg[ind]): 
                x = 10
            if torch.isinf(msg[ind]): 
                x = 10
        return msg.view(-1, 1) 
    ...
    def update(self, aggr_out, x):
        ...
        for msg, node in zip(aggr_out, x):
            node = node.clone()         
            node[1] = node[1] + msg[0]  
            if int(node[0].item()) in self.proj_dict.keys():
                node[1] = self._projection_(node)
            new_nodes.append(node)
        return torch.stack(new_nodes, dim=0)
